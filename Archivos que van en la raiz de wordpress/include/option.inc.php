 ...
 */
  ...
  while ($registrProy = $resultact->fetch_array( MYSQLI_BOTH)) {
	...
	$sql = "SELECT c.IDCURSO AS 'IDC', C.NOMBRE AS 'CNOMBRE', A.NOMBRE AS 'ANOMBRE' FROM curso C INNER JOIN area A ON C.IDAREA = A.IDAREA WHERE C.IDPROGRAMA = ".$codProy."  AND C.NECESITAPRERREQUISITOS = 0 AND (c.CUPO_LIMITE > 0 OR c.CUPO_LIMITE IS NULL) ORDER BY A.NOMBRE";
