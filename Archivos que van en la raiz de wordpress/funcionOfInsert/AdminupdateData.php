...
$sqlDataPersonal = sprintf("UPDATE `estudiante` SET `NOMBRE`= '%s',`SEGUNDONOMBRE`= '%s',`APELLIDO`= '%s',`SEGUNDOAPELLIDO`= '%s',`TELEFONO`= %s,`CORREO`= '%s',`FECHANAC`= '%s',`SEXO`= '%s' WHERE `DPI_ESTUDIANTE`= %s",
...
echo '<br/><form method="post" action="http://localhost/wordpress/formulario-de-asignaciones-administrador/" class="form-horizontal"><div style="text-align:center"> <input type="hidden" name="cui" value="'.$dpi.'"><input type="submit" id="btnAsignar" name="btnAsignar" class="btn btn-primary" value="Ir Asignarse Curso"></div></div></form>';
//
